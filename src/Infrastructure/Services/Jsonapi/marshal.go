...
// The MarshalCustomLinks interface can be implemented if the struct should
// want any custom links.
type MarshalDocumentCustomLinks interface {
	MarshalIdentifier
	GetDocumentCustomLinks(string) Links
}

// The MarshalMeta interface can be implemented if the struct should
// want any meta.
type MarshalDocumentMeta interface {
	MarshalIdentifier
	DocumentMeta() Meta
}
...
func Marshal(data interface{}, globalMeta Meta) ([]byte, error) {
...
func marshalSlice(data interface{}, information ServerInformation) (*Document, error) {
	...
	elementsLen := val.Len()
	dataElements := make([]Data, elementsLen)
	var referencedStructs []MarshalIdentifier
	...
	for i := 0; i < elementsLen; i++ {
		...
		if casteMetaTarget, ok := element.(MarshalDocumentMeta); ok {
			/*
				meta := casteMetaTarget.DocumentElementMeta()
				result.Meta[element.GetID()] = meta
				generalMeta := casteMetaTarget.DocumentGeneralMeta()
				logger.Debugf("::: GENERAL METADATA ::: %v", generalMeta)
				for key, value := range generalMeta {
					result.Meta[key] = value
				}
			*/

			meta := casteMetaTarget.DocumentMeta()
			if meta != nil {
				for k, v := range meta {
					result.Meta[k] = v
				}
...
func marshalData(element MarshalIdentifier, data *Data, information ServerInformation) error {
	...
	//if no attributes serialized ignore "attributes" field
	if bytes.Compare(attributes, encodedEmptyObjet) != 0 {
...
func marshalStruct(data MarshalIdentifier, information ServerInformation) (*Document, error) {
	...
	if information != nil {
		if customLinks, ok := data.(MarshalDocumentCustomLinks); ok {
			if result.Links == nil {
				result.Links = make(Links)
			}
			base := getLinkBaseURL(data, information)
			for k, v := range customLinks.GetDocumentCustomLinks(base) {
				if _, ok := result.Links[k]; !ok {
					result.Links[k] = v
				}
			}
		}
	...
	if casteMetaTarget, ok := data.(MarshalDocumentMeta); ok {
		meta := casteMetaTarget.DocumentMeta()
		result.Meta = meta
	}
